...
def build():
    ...
    env = VectorizedHamletEnv(
        universe=u, level_name=LEVEL, num_agents=1, device=torch.device("cpu")
    )
    return u, env
...
def main() -> None:
    ...
    print(f"positions tensor shape : {tuple(env.positions.shape)}  <- agent dim = "
          f"{env.positions.shape[0]}")
    ...
    print(f"NOT a union of agent positions: positions tensor is "
          f"{tuple(env.positions.shape)}, a different variable")
    ...
    print(f"GROW positions declared in YAML    : "
          f"{len(lvl.affordances.affordances[0].deployment.positions)}")
    print(f"GROW positions deployed at runtime : 1 (tensor shape "
          f"{tuple(env.affordances['GROW'].shape)})")
    ...
    if non_adj:
        ...
        print(f"    example: prev={DECLARED_ROOT} new={worst} "
              f"manhattan={min(manhattan(worst, p) for p in prev)}")
    ...
    print(f"both arms actually grew (count > 1)   : "
          f"{len(set_a) > 1 and len(set_b) > 1}")
    print(f"sets DIFFER (required for B-F6)       : {set_a != set_b}")
    print(f"symmetric difference size             : "
          f"{len(set(set_a) ^ set(set_b))}")
    ...
    print(f"WAREHOUSE pos={list(DECLARED_WAREHOUSE)} "
          f"interacted={i_wh['successful_interactions']}")
    print(f"          biomass {b2} -> {b3}   reward vector = {r_wh.tolist()}")
    print(f"meter delta at warehouse {b3 - b2:+.4f}   control {b1 - b0:+.4f}")
    print(f"reward delta (warehouse - control) : "
          f"{float(r_wh[0]) - float(r_ctrl[0]):+.6f}")
    ...
    for f in spec.fields:
        print(f"  {f.name:24s} dims={f.dims:4d} [{f.start_index}:{f.end_index}] "
              f"sem={f.semantic_type:10s} feature={f.feature}")
    ...
    print(f"encodes declared coordinate  : "
          f"{[float(x) for x in wv] == [float(x) for x in DECLARED_WAREHOUSE]}")
    ...
    for f in spec.fields:
        ...
        if float(seg.min()) < 0.0 or float(seg.max()) > 1.0:
            print(f"  OUT OF [0,1]: {f.name} -> min={float(seg.min()):.3f} "
                  f"max={float(seg.max()):.3f}")
    ...
    for ep in range(5):
        ...
        for _ in range(20):
            ...
            if bool(d[0]):
                term = True
                break
        print(f"  ep{ep}: steps={len(rs)} terminated={term} "
    ...
    print(f"shaping            : "
          f"{[(s.type, getattr(s, 'target_affordance', None)) for s in d.shaping]}")
