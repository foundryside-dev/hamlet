...
| [`config-schemas/`](config-schemas/) | Config reference per file type — and **the tier to be most careful with**. Fully audited against source 2026-08-26: only **`bars.md`, `presentation.md` and `transition_rules.md` verified clean**. The other **10 carry dated banners**, several severe — `affordances.md` documents an unwired schema, `vfs-profiles.md` omits both required top-level fields, `brain.md` has 3 of 4 examples DTO-rejected, and `expressions.md` labels 9 shipped functions "planned". **Read the banner before the body**, and prefer `src/townlet/config/` DTOs as the authority. |
