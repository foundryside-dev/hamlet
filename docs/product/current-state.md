...
| 3 | `Gates green` on a suite that hides nothing (`PDR-0059`) | met on `main` at `4222a917` (per-push Tests green); **the first post-merge nightly (06:00 UTC 08-17) is still the reading owed** — `PDR-0072` trigger 2 fires if red on the three named files |
...
- One CI red on the push to `8c5fa2c8`: the repo's `no_defaults_lint` gate caught a ternary-as-default in the
  new sync step; fixed in this checkpoint's commit. Lesson kept: run all five Lint-workflow checks locally,
  not four.
...
- `PDR-0072` trigger 2: still live on the first post-merge nightly. `PDR-0043` trigger 2: discharged.
...
**Read the first post-merge nightly** (`gh run list --workflow full-tests.yml --limit 1`, the run after
2026-08-16T06:33Z) — green closes the `PDR-0059` thread; red fires `PDR-0072` trigger 2. Then pick the next
WS-4 unit on `PDR-0019`'s criterion (*where does the runtime still know what the game is?*): the sibling
