...
|--------|-----------------|---------|---------|
| **Gates green** — `ruff check`, `black --check`, `mypy src`, `pytest` | all 4 pass | **1 of 4 — MEASURED, `PDR-0010`.** ruff **pass**; black **fail** (`environment/dac_engine.py`, +3 under `tests/`); mypy **fail** (3 errors in 2 files); pytest **fail** (1 of 2942 — a stale benchmark fixture, not a product defect). Recorded green since 2026-05-16 while three of four were red. Nothing here is deep; it went unnoticed *because the workspace said it was green*. | 2026-08-11 |
