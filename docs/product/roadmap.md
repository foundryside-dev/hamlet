...
- **Measure the authoring claim — the instrument, not another authoring unit** (`PDR-0077`,
  ...
  **drawn mechanically** (`PDR-0080`), stratified across seven axis buckets, seeded by the corpus's
  ...
  same 2026-10-06 clock — two trial packs now sit on it). Next draw: any of B, D, E, J, K, M, O
  — one per session; blind re-runs (criterion 3, 2 needed) are now possible with two records on
  file, chosen by the comparer, run at each trial's pinned commit.
