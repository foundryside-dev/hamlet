...
def test_custom_item_verbs_flow_from_config_to_runtime(tmp_path):
    ...
    action_names = {a.name for a in universe.get_level("L0_smoke").action_metadata.actions}
    ...
    spawn_position = next(
        (x, y) for x in range(env.substrate.width) for y in range(env.substrate.height) if (x, y) not in occupied_positions
    )
