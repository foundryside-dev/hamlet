...
def test_evaluation_is_marked_by_declaration_not_by_exposure():
    ...
    profiles_config = VFSProfilesConfig(version="1.0", evaluation_mode="mark_and_sweep", debug_logging=False, global_profile=profile)
    ...
    assert VFSCompiler().derive_evaluation_marks(profiles_config, overlay_variables=(_overlay_var("derived", observable=True),)) == {
        "global": {"derived"}
    }
...
def test_overlay_observable_on_a_static_never_marks_it():
    ...
    marks = VFSCompiler().derive_evaluation_marks(profiles_config, overlay_variables=(_overlay_var("counter", observable=True),))
