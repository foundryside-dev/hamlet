...
def test_full_auto_keeps_fields():
    ...
    out = UniverseCompiler._apply_observation_mode(fields, mode)
...
def test_full_manual_requires_known_fields():
    ...
    reordered = UniverseCompiler._apply_observation_mode(fields, reorder)
    assert [f.name for f in reordered] == ["y", "x"]

    bad_mode = ObservationModeConfig(mode="full_manual", include_fields=["z"])
    with pytest.raises(ValueError):
        UniverseCompiler._apply_observation_mode(fields, bad_mode)
