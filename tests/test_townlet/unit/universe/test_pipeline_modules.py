...
def test_load_v21_configs_loads_root_optional_artifacts(tmp_path: Path) -> None:
    ...
    assert bundle.raw.vfs_profiles is not None
    assert bundle.raw.effects is not None
    assert bundle.raw.action_label_overrides == {0: "PORT"}
    assert bundle.raw.variables_reference is not None
    assert {var.id for var in bundle.raw.variables_reference} == {"position"}
